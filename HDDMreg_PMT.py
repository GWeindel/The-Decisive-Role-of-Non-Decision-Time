...
def run_model(id):
    ...
    if id < 4:
        ...
        LM = [{'model':'t ~ SAT  + FC + contrast + SAT:FC + SAT:contrast + FC:contrast + SAT:FC:contrast', 'link_func': lambda x: x} ,
			  ...
			  {'model':'z ~ FC', 'link_func': lambda x: x}]
	deps = {'sz' : 'SAT'}
        inc = ['sv','sz','st','z']
    else :
        return np.nan()
    name = 'reg_PMT_M13_%s' %str(id)
    ...
    m.sample(iter=35000, burn=32500, thin=1, dbname='DDM/traces/db_%s'%name, db='pickle')
    m.save('DDM/Fits/%s'%name)
    return m

v = ipyparallel.Client(profile="reg_PMT_long")[:]#sept
