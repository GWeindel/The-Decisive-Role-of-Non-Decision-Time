...
data = pd.read_csv('data_pre_G-LME.csv')

GLMEdata = data.copy()
GLMEdata['FC'] = GLMEdata.apply(lambda row: -0.5 if row['FC'] == 1 else 0.5, axis=1)
print(GLMEdata.FC.unique())
GLMEdata['contrast'] = GLMEdata.contrast.replace([1,2,3,4,5,6], [-.5,-.3,-.1,.1,.3,.5])
GLMEdata['SAT'] = GLMEdata.apply(lambda row: 0 if row['SAT'] == 'Accuracy' else 1, axis=1)
GLMEdata['participant'] = [int(x.split('S')[-1]) for x in GLMEdata.participant]
